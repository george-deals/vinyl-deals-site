...
const ALLOWED_MEDIA_TYPES = new Set(["vinyl", "cd", "4k-uhd", "blu-ray", "dvd"]);
...
export async function GET(req: Request) {
  ...
  if (!ALLOWED_MEDIA_TYPES.has(mediaType)) {
    return new Response(
      JSON.stringify({ ok: false, error: "Invalid media_type", allowed: Array.from(ALLOWED_MEDIA_TYPES) }),
  ...
  // Pull oldest-seen first so we rotate coverage within each media type
  ...
  // Update last_seen_at for the ASINs we attempted (scoped to this media type)
  if (asins.length) {
